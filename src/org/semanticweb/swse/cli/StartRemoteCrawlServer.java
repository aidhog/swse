...
/**
 * Main method to setup a crawler which can be run via RMI.
 * 
 * @author aidhog
 */
public class StartRemoteCrawlServer {
	...
	public static void main(String args[]) throws IOException, ClassNotFoundException, AlreadyBoundException{
		...
		try {
			...
			System.err.println("Don't forget to set 'ulimit -n 65536' and '-DentityExpansionLimit=512000'");
		...
		int port = RMICrawlerConstants.DEFAULT_RMI_PORT;
		if (cmd.hasOption("p")) {
		...
		RMICrawlerServer.startRMIServer(hostname, port, RMICrawlerConstants.DEFAULT_STUB_NAME);
